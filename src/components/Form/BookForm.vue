      ...
      <a-row :gutter="24">
        <a-col :span="9">
          <a-flex vertical :gap="10" align="center">
            ...
            <a-avatar v-else :size="300" shape="square">
...
const handleFileUpload = async (event) => {
  try {
    ...
    if (file) {
      ...
      const { data } = await uploadImage('book', file)
      formState.value.image = data.imagePath
    }
  } catch (err) {
    console.log(err)
  }
...
const getImg = computed(() => {
  ...
  if (props.initState.image) return baseURL + '/' + props.initState.image
  return false
})
</script>
